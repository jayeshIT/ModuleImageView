...
#import "ComTestmoduleView.h"

@implementation ComTestmoduleView
...
-(void)willMoveToSuperview:(UIImageView *)newSuperview
{
...
-(UIImageView*)square
{
	...
	if (square == nil) {
		...
		square = [[UIImageView alloc] initWithFrame:[self frame]];
...
-(void)setBackgrounImage_:(id)myimage
{
    
    NSString *imagePath = [self getPathToModuleAsset:myimage];
	UIImage *image = [UIImage imageWithContentsOfFile:imagePath];
	if (image == nil) {
		return nil;
	}
    
    UIImageView *im = [self square];
    im.image=image;
}
